...
// !!! =============================================================== !!!
const SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbwf6FAmaQnQ83E0Ab12C2kwS1_jqETQSoi51S-ukVyHGMjgWIjI27Dg2leYZJHEcIzs/exec';
...
async function postToAction(action: string, payload: any) {
  try {
    const response = await fetch(SCRIPT_URL, {
      method: 'POST',
      mode: 'no-cors', // Keep no-cors for simplicity with Apps Script redirects
      headers: {
        'Content-Type': 'text/plain;charset=utf-8', // Required for no-cors to send complex data
      },
...
function AppContent() {
  ...
  const handleAddFeedback = async (newItem: Omit<FeedbackItem, 'id' | 'timestamp' | 'comments'>): Promise<boolean> => {
    // The ID is now generated by the backend, so we don't create it here.
    const item = {
      ...newItem,
      timestamp: Date.now(),
      comments: [],
    ...
    const { success } = await postToAction('addFeedback', item);
    if (success) {
      await new Promise(resolve => setTimeout(resolve, 2000)); // Give sheet time to update
  ...
  const handleAddComment = async (itemId: string, newComment: Omit<Comment, 'id' | 'timestamp'>): Promise<boolean> => {
    ...
    const { success } = await postToAction('addComment', payload);
    if (success) {
  ...
  const handleUpdateDevResponse = async (itemId: string, response: Omit<DevResponse, 'timestamp'>): Promise<boolean> => {
    ...
    const { success } = await postToAction('updateDevResponse', payload);
     if (success) {
      await new Promise(resolve => setTimeout(resolve, 2000));
      await fetchFeedbackData();
    } else {
       alert('更新開發者回覆失敗，請稍後再試。');
    }
    return success;
  };
